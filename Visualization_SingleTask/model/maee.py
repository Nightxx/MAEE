...
class Maee(Layer):
    ...
    def __init__(self,
        ...
        # tensor output
        self.expert_representations = []
        self.expert_update_gate = []
        self.field_tag = []
        # Activity parameter
    ...
    def call(self, inputs, **kwargs):
        ...
        for i in range(self.num_fields):
            ...
            for j in range(self.field_expert_num_list[i]):
                ...
                expert_representations.append(expert_output)
                field_experts.append(expert_output)
                if self.field_expert_type_list[i] == "discrete":
                    mask_indices = self.field_expert_index_list[i][j]
                    mask = tf.greater(tf.zeros_like(inputs[:,mask_indices[0]], dtype=tf.float32), 1)
                    for mask_index in mask_indices:
                        mask = tf.logical_or(mask, tf.greater(inputs[:,mask_index], 0.0))
                    mask_value = tf.expand_dims(mask, axis=1)
            ...
            mask_value_list.append(mask_list_concat)
            ...
            attention_value_outputs.append(reverse_masked_attention_weights)
        ...
        expert_representations.append(common_expert_output)
        gate_outputs = []
        ...
        final_outputs.append(tf.stack(expert_representations, axis=2))
        final_outputs.append(tf.concat(update_outputs, axis=1))
        final_outputs.append(tf.concat(mask_value_list, axis=1))
